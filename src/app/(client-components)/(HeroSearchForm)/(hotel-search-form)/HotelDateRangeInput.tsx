...
export interface HotelDateRangeInputProps {
  className?: string;
...
const HotelDateRangeInput: FC<HotelDateRangeInputProps> = ({
  className = "",
      ...
      <>
        ...
        <div className="flex-grow text-left">
          ...
          <span className="block mt-1 text-sm text-neutral-400 leading-none font-light">
            {selectsRange ? "Check In - Check Out" : "Check In date"}
          </span>
        </div>
      </>
    );
    ...
    <>
      <Popover className={`FlightDateRangeInput relative flex ${className}`}>
        {({ open }) => (
          <>
            <div
              className={`flex-1 z-10 flex items-center focus:outline-none ${open ? "nc-hero-fi eld-focused" : ""
                }`}
            >
              <Popover.Button
...
export default HotelDateRangeInput;
