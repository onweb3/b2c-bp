...
interface Destination {
  ...
  // Add any other properties you have in the object
}
...
const VisaDestination: FC<LocationInputProps> = ({
  autoFocus = false,
  ...
  const visaDetination = async () => {
    try {
      const destination = await fetch(
        `${config?.NEXT_PUBLIC_SERVER_URL}/api/v1/visa/all`
      );
  ...
  async function getVisaDestination() {
    try {
      const response = await visaDetination();
      {
        response?.map((destination: any, index: number) =>
          setValue(destination?.country?.countryName)
        );
      }
  ...
  useEffect(() => {
    {
      destination?.map((destination: any, index: number) =>
        setVisaDestination?.(destination?.slug)
      );
    }
  }, [value]);

  const renderRecentSearches = (value: string, destination: Destination[]) => {
    return (
      <>
        {value?.length > 0 && (
        ...
        <div className="mt-2">
          {destination?.map((item) => (
            <span
              onClick={() => handleSelectLocation(item?.country?.countryName)}
              key={item?._id}
    ...
    <div className={`relative flex ${className}`} ref={containerRef}>
      ...
      {showPopover && (
        <div className="absolute left-0 z-40 w-full min-w-[300px] sm:min-w-[500px] bg-white dark:bg-neutral-800 top-full mt-3 py-3 sm:py-6 rounded-3xl shadow-xl max-h-96 overflow-y-auto">
          {value?.length > 2 ? (
            <>
              {renderRecentSearches(value, destination)}
              {renderSearchValue()}
            </>
          ) : (
            renderRecentSearches(value, destination)
