    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className=" max-w-xl space-y-6">
        ...
        <div>
          ...
          <Input value={password} onChange={(e) => setPassword(e?.target?.value)} type="password" className="mt-1.5" />
        </div>
        <div>
          <Label>Confirm password</Label>
          <Input value={confirmPassword} onChange={(e) => setConfirmPassword(e?.target?.value)} type="password" className="mt-1.5" />
        </div>
        <div className="pt-2">
        {email !== "" && password !== "" && confirmPassword !== "" && (
          <ButtonPrimary onClick={accountDelete}>Remove Account</ButtonPrimary>
        )}
          {error !== "" && (
            <p className="text-[13px] mt-3">{error}</p>
          )}

{message !== "" && (
            <p className="text-[13px] mt-3">{message}</p>
          )}
        </div>
      </div>
    </div>
