        ...
        <div className="columns-1 gap-4 sm:columns-2 xl:columns-3">
          {images.map((img, ind) => {
            ...
            if (img instanceof Object) {
              if (img.isRelative) {
                currentImage = `${config?.NEXT_PUBLIC_CDN_URL + img.path} `
              } else {
                currentImage = img.path
              }
            } else if (typo == "string") {
              if (img.startsWith("/public/images")) {
                currentImage = `${config?.NEXT_PUBLIC_CDN_URL + img} `
              } else {
                currentImage = img
              }
            }
            ...
            return (
              <div
                key={ind}
                onClick={() => {
                  const newPathname = getNewParam({ value: ind });
                  router.push(`${thisPathname}/?${newPathname}` as Route);
                }}
                ref={ind === Number(lastViewedPhoto) ? lastViewedPhotoRef : null}
