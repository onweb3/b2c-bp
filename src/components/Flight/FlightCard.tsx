...
const FlightCard: FC<FlightCardProps> = ({ className = "", data, tripType }) => {
  ...
  const renderDetailRoundTrip = () => {
    if (!isOpen || tripType !== "round-trip") return null;
    return (
  ...
  const renderDetailOneWay = () => {
    if (!isOpen || tripType !== "one-way") return null;
    return (
      <div className="p-4 md:p-8 border border-neutral-200 dark:border-neutral-700 rounded-2xl ">
        {renderDetailTop()}
      </div>
  ...
  return (
    ...
    >
      <div className={` sm:pr-20 relative  ${className}`}>
        ...
        <a hrefLang={"en"}href="##" className="absolute inset-0" />
